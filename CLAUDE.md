...
- **Background agents** (v26): `agent_runs` is the persistent control plane for background `delegate` runs (stoppable from Settings); `checkpoints` stores pre-edit file snapshots for reversible code changes. Project hooks are declarative DB rows executed behind the same approval boundary as tools. Since v32 an **agent profile** (`agents`, v21) is an owner, not just a persona: a scheduled task names the agent that runs it (`scheduled_tasks.agent_id`, also settable from the `schedule_task` tool's `agent` argument), and `memories.agent_id` gives that agent its own recollection — unique per owner by title; a bot reads its own memories plus the user's shared pool (never another bot's) and writes only its own, and dreaming consolidates each namespace (shared pool, each bot) in its own model call so one agent's facts can never be merged into another's.

**Bot Mode** (v46, Hermes Desktop-style): agent profiles double as a roster of named **bots** (Bots view, `ui.view = 'bots'`). v46 adds `agents.title/avatar_json/hidden/chat_conversation_id`; a bot's **canonical chat** is a real conversation (`conversations.agent_id`, hidden from the sidebar listing — backup opts back in with `includeBots`) that rides the ordinary interactive `send()` pipeline: `resolveTarget` falls back to the bot's model pin, `buildHistory` injects the persona + the bot's OWN memories + the Bot Mode roster/protocol section (`services/bot-prompts.ts`), `planTools` applies the bot's toolset and offers `message_agent` ONLY there, and the memory completion hook writes ` ```uld-memory ` directives to the bot's own memory. `BotService` (`src/main/services/bots.ts`) owns everything else: fire-and-forget bot-to-bot deliveries (queued per target, reply routed back via the completion hook, one transient retry, typed failure reasons, hop cap 6), **group rooms** (`bot_groups`/`bot_group_members`; one shared transcript conversation, `messages.agent_id` attribution, up to 3 serial reply-or-pass rounds per user send via tool-less `generateForWorkflow`, 10-message cap, @mention scoping, `@user` escalation → `needs_user` badge), and routine mirroring (a scheduled task with `agent_id` reports its result into the bot's chat). Deleting a profile cleans up its chat + memberships app-side (no FKs anywhere in Bot Mode).

**Bot gateway** (v47, OpenClaw-inspired): the layer between a bot and the outside world. Per-bot **heartbeats** (`agents.heartbeat_json`; a periodic open-ended turn in the canonical chat where `NO_REPLY` deletes the turn — BotService's 60 s maintenance tick, deferred while busy, ≥15 min cadence) and **auto-compact policies** (`reset_json`, daily-at-hour / idle-minutes → `compactNow`, never deletion); a per-bot `message_agent` **allowlist** (`message_allow_json`; NULL = open, `[]` = off); room **activation modes** (`bot_groups.activation`: 'mention' = only @named bots take one turn, no open rounds) and **observer members** (`bot_group_members.observer`: read the room, speak only when @mentioned); a room send during settling queues and drains as one coalesced round-set. The **busy-send message queue** in `ChatService` (`send(req, {queueIfBusy: true})` — the chat IPC handler opts in): a send into a busy conversation persists + queues (cap 20), and one coalesced follow-up turn starts 500 ms after the stream COMPLETES (never after Stop/error; any starting turn clears the counter since history covers it). `fetch_url`/`web_search` output is wrapped in untrusted-content boundary markers (`services/untrusted.ts`); scheduled tasks can POST results to a per-task `webhook_url` (`services/task-webhook.ts`, https/localhost-only). **Per-bot Telegram bindings** (`bot_bindings` + `src/main/im/bot-channels.ts`): each bot can run its own Telegram bot (token encrypted in `tool_secrets`, owner `bot:<agentId>`), DM-paired trust-on-first-use; groups are allowlist-only with mention gating, managed by owner-only in-group commands (`/allowgroup`, `/denygroup`, `/activation mention|always` — the owner is verifiable because a Telegram private-chat id equals the user id). Inbound messages become canonical-chat turns (queue-if-busy); replies flush back through the completion hook (`TelegramBridge.onInbound` is the enriched path; the legacy owner bridge is untouched).

**Durable deliveries + attention** (v48/v49): `message_agent` deliveries are `a2a_outbox` rows (`src/main/db/repositories/a2a-outbox.ts`) pumped per target strictly FIFO by `BotService` — queued → delivered (turn running; `target_conversation_id`/`assistant_message_id` set) → replied | failed | cancelled, with `hop` and `attempts` persisted so the chain cap and the single retry/redelivery budget survive restarts; the pump is a single-flight loop with a pump-again flag, status + reply row commit in one transaction, and `BotService.recover()` runs at boot right after `markDanglingStreamingAsStopped`. The sender's chat gets a `system`-role handoff marker carrying `messages.handoff_json` (`MessageHandoff`; `buildHistory` and compaction skip system rows, so the model never re-reads it) rendered by `HandoffCard`; the target's incoming turn and the routed reply carry the same chrome. Roster attention (`BotAttention`: needs_you > unread > working > idle) is computed in `BotService.roster()` from `agents.chat_seen_at`/`bot_groups.seen_at` (stamped by `bots:markSeen` while the window is focused on that chat), `PendingBroker.hasPendingFor`, the outbox, running `agent_runs.agent_id` and routines; `attentionCount()` joins the inbox count in the dock/tray badge, and notification clicks deep-link through `push:navigate` (held while locked). One editor for one object: `renderer/components/agents/AgentProfileForm.tsx` serves both the Bots pane and Settings → Agents.

Bot Mode caps (rounds/messages per room send, hop depth, room size) are `AppSettings.botMode` (v50, clamped by `resolveBotModeLimits` in `bot-prompts.ts`; the Hermes numbers by default), and round-table participants of one round answer in parallel with replies posted in member order.
